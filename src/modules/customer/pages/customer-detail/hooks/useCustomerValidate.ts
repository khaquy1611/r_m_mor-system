...
const useCustomerValidate = () => {
  ...
  const customerValidate = Yup.object({
    name: Yup.string()
      .textRequired('Customer Name required to have input')
      .customerNameValidation(
        'Customer Name cannot contain unauthorized character. Please update this information'
      ),
    abbreviation: Yup.string()
      .textRequired('Customer Abbreviation required to have input')
      .matches(
        abbreviationRegex,
    ...
    // website: Yup.string()
    //   .trim()
    //   .matches(urlWebsiteRegex, 'Customer Website is not valid'),
    ...
    marketId: Yup.string().required('Market must be selected'),
    scale: Yup.string()
      .trim()
      .scaleValidation(
