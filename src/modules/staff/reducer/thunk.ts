...
export const getStaffHeadcountUsed = createAsyncThunk(
  'staff/getStaffHeadcountUsed',
  async (payload: StaffHeadcountUsedType, { rejectWithValue }) => {
    try {
      const res = await StaffService.getStaffHeadcountUsed(payload)
      return res
    } catch (err: any) {
      return rejectWithValue(err)
    }
