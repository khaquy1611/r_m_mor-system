...
import { filterFollowKeyword } from '@/utils'
    ...
    >
      ...
      >
            ...
            fallback={
              ...
              >
                <Box
                  ...
                  component="span"
                  className={'ellipsis value-single'}
                  title={Array.isArray(value) ? '' : value?.label ?? ''}
                >
        ...
        <Box className={classes.buttonWrapper}>
          {!!listSelected.length && isShowCloseIcon && (
            <Box
        ...
        <Box className={classes.listWrapper} style={styleListDropdown}>
          <ConditionalRender conditional={isShowDropdown}>
            <ConditionalRender
              conditional={
                !!listOptionDisplay.length ||
                (!loading && !!listOptionDisplay.length)
              }
              fallback={
                <Box className={classes.noData}>
                  {loading ? 'Loading...' : i18nCommon('MSG_NO_DATA_AVAILABLE')}
                </Box>
              }
            >
              <Box
                component="ul"
                className={clsx(classes.listOptions)}
                onScroll={onScroll}
                ref={optionListRef}
              >
...
const ChipSelected = ({ item, onClickOption }: any) => {
  const classes = useStyles()
  return (
    <Box className={clsx(classes.selectedItem)}>
      <Box component="span" className={'ellipsis'} title={item.label}>
        {item.label}
      </Box>
      <CloseIcon onClick={() => onClickOption(item)} />
    </Box>
  )
}
...
const DropdownItem = ({
  ...
  return (
    <Box
      ...
      title={option.label}
    >
      <Box className="ellipsis">{option.label}</Box>
